...
<html >
	...
	<body >
		...
		<form action="../deal/waimai_action.php" method="POST" id="tijiao">
	    ...
	    <div class="juli"></div>
	     <ul class="list-detail">
	    	<li class="time">
	    		<span>优惠券：</span>
	    		<input type="text" onclick="window.location.href='youhui.php?money=<? echo cart::$all_price;if($did != null){ echo "&did={$did}";}if($id != null){ echo "&id={$id}";}?>'" name="order_people" <? if($yid == null){ echo 'placeholder="点击选择优惠券"';}else{ echo 'placeholder ="'; echo $youhui_val; echo '"'; } ?> required>
	    		<i class="am-icon-angle-right"></i>
	    	</li>
	    	<li class="time">
	    		<span>代金券：</span>
	    		<input type="text" onclick="window.location.href='daijin.php?money=<? echo cart::$all_price; if($yid != null){ echo "&yid={$yid}";}if($id != null){ echo "&id={$id}";}?>'"  <? if($did == null){ echo 'placeholder="点击选择代金券"';}else{ echo 'placeholder ="'; echo $daijin_val; echo '"'; } ?> required>
	    		<i class="am-icon-angle-right"></i>
	    	</li>
	    ...
	    <ul class="list-detail">
				...
				<li class="time">
					<span>收货地址：</span>
					<input type="text" name="sh_address" placeholder="选择您的收货地址" onclick='window.location.href="gladd.php?<? if($yid != null){ echo "yid={$yid}";} if($yid != null && $did != null){ echo "&did={$did}";}else if($yid == null && $did != null){ echo "did={$did}";} ?>"' value="<?sh_message($id,'1');?>">
					<i class="am-icon-angle-right"></i>
				</li>
				<li class="time">
					<span>电话号码：</span>
					<input type="text" name="tel" placeholder="请输入您的电话号码"  value="<?sh_message($id,'2');?>">
					<i class="am-icon-angle-right"></i>
				</li>
				<li class="time">
					<span>您的姓名：</span>
					<input type="text" name="order_name" placeholder="请输入您的姓名"  value="<?sh_message($id,'3');?>" />
					<i class="am-icon-angle-right"></i>
				</li>
				<!--
				<li class="time">
	...
	$pay_money = 0;
	 ...
	 <!--   <div class="am-modal am-modal-no-btn" tabindex="-1" id="doc-modal-1">
	    ...
	    <div class="juli"></div>
	    <div class="pricebox">
	    	<p >总价：<i><?echo round($pay_money*$zhe,2);?></i>元（<em><?echo cart::$all_num;?></em>份）</p>
		...
		<a href="index.php">
		<input type="button" class="btn" onclick="" value="重新选择" style="background:#999999;margin-top:-5px;">
	</a>
	</div>
	</form>
	<!-- 支付密码输入弹出-->

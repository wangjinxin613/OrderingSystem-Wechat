        ...
        <script type="text/javascript">
            $(function () {
                (function longPolling() {
                    ...
                    $.ajax({
                        url: "comet/Orderform.php",
                        data: {"timed": Date.parse(new Date())/1000},
                        dataType: "text",
                        timeout: 5000,//5秒超时，可自定义设置
                        error: function (XMLHttpRequest, textStatus, errorThrown) {
                            $("#state1").append("[state: " + textStatus + ", error: " + errorThrown + " ]<br/>");
 ...
 <div class="cover_style" id="cover_style">
    ...
    <div class="search_style">
     ...
     <div class="border clearfix search_style">
       <span class="l_f">
        <a href="?type=1<? if($fen_id != null) { echo "&fen_id=$fen_id";}?>"  class="btn btn-warning">今日新增</a>
				<a href="?type=2<? if($fen_id != null) { echo "&fen_id=$fen_id";}?>"  class="btn btn-warning">本周新增</a>
				<a href="?type=3<? if($fen_id != null) { echo "&fen_id=$fen_id";}?>"  class="btn btn-warning">本月新增</a>
      	<a href="?type=4<? if($fen_id != null) { echo "&fen_id=$fen_id";}?>" class="btn btn-warning">本年新增</a>
      	 	<a href="phpexcel/order_list.php?type=<? echo $type;?><? if($fen_id != null) { echo "&fen_id=$fen_id";}?>"  class="btn">导出以下数据</a>
       </span>
	...
	<tbody >
   		<?php 
   			if($time == null){
   				order_list($admin_sql1,$type,$fen_id);
   			} 
   			else{ 
   				order_list_time($time,$fen_id); 
   			}
   			?>
     </tbody>
     </table>

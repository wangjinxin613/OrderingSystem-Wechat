<?php
    ...
    if(check_dept('18') == false){
        exit('<script>alert(\'您没有权限访问本页面\');history.back();</script>');
    }
    $status = $_GET['status'];
    $status_ss = $_GET['status_ss'];

    if($status != null){
    if(ctype_digit($status) == false){ //检测传递过来的参数必须是数字，防止被黑客攻击
        exit('<script>alert(\'参数传递有误！！\');history.back();</script>');
    }
  }
    if($status_ss != null){
    if(ctype_digit($status_ss) == false){ //检测传递过来的参数必须是数字，防止被黑客攻击
        exit('<script>alert(\'参数传递有误！！\');history.back();</script>');
    }
  }
   $fen_id = $_GET['fen_id'];
  if($fen_id != null){ $sq=" and fen_id = $fen_id";$sq1=" where fen_id = $fen_id";}
?>
...
<div class="margin clearfix">
 <div class="cover_style" id="cover_style">
   ...
   <div class="centent_style" id="centent_style">
       ...
       <div id="scrollsidebar" class="left_Treeview">
        ...
        <div class="widget-box side_content" >
         ...
         <ul class="b_P_Sort_list">
             <li><i class="orange  fa fa-reorder"></i><a href="?type=0<? if($fen_id != null){ echo "&fen_id=$fen_id";}?>">全部订单(<? tj("orderlist {$sq1}");?>)</a></li>
             <li><i class="fa fa-sticky-note pink "></i> <a href="?status=0<? if($fen_id != null){ echo "&fen_id=$fen_id";}?>">未支付(<? tj("orderlist where status = 0 {$sq}");?>)</a> </li>
             <li><i class="fa fa-sticky-note pink "></i> <a href="?status=1<? if($fen_id != null){ echo "&fen_id=$fen_id";}?>">已支付(<? tj("orderlist where status = 1 {$sq}");?>)</a></li>
             <li><i class="fa fa-sticky-note pink "></i> <a href="?status_ss=0<? if($fen_id != null){ echo "&fen_id=$fen_id";}?>">待处理(<? tj("orderlist where status_ss = 0 {$sq}");?>)</a></li>
             <li><i class="fa fa-sticky-note pink "></i> <a href="?status_ss=1<? if($fen_id != null){ echo "&fen_id=$fen_id";}?>">成功订单(<? tj("orderlist where status_ss = 1 {$sq}");?>)</a></li>
             <li><i class="fa fa-sticky-note pink "></i> <a href="?status_ss=2<? if($fen_id != null){ echo "&fen_id=$fen_id";}?>">失败订单(<? tj("orderlist where status_ss= 2 {$sq}");?>)</a></li>
            </ul>
       </div>
      </div>
     </div>
	...
	<tbody>
    <?order_list1($admin_sql1,$status,$status_ss,$fen_id);?>

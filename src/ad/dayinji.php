...
<?
	...
	function test(){
		global $user;
		$result = mysql_query("SELECT * FROM dayinji where store_id = '$user->store_id' ");
	while($row = mysql_fetch_array($result))
 	{ 
 		echo 'd'; echo $row['name'];
 	}
	}
...
<div class="page-content clearfix">
  <div class="search_style">
    ...
    <ul class="search_content clearfix">
     ...
     <li style="float:right">
      <input type="button" value="获取当前可使用的打印机名称" onclick="document.getElementById('T3').value=getPrinterName()">    <input type="text" id="T3" size="30">
  </li>
    </ul>
 ...
 <div class="sort_style">
     <div class="border clearfix">
       <span class="l_f">
        <a href="javascript:ovid()" id="sort_add" class="btn btn-warning"><i class="fa fa-plus"></i> 添加打印机</a>
    
       	<a style="color:red;font-size:18PX;margin-left:50px;">*打印机名称不可重复，打印机名字可到开始菜单->设备与打印机进行设置	</a>
       </span>
...
<div class="sort_style_add margin" id="sort_style_add" style="display:none">
  <div class="">
     <ul>
     	<form action="action/dayinji_add.php" method="POST">
      <li><label class="label_name">当前门店</label><div class="col-sm-9">
         &nbsp;&nbsp;<? fendian_odd($fen_id);?>
         <input type="text" value="<? echo $fen_id;?>" name="fen_id" style="display:none">
      </div></li>
      <li><label class="label_name">打印机名称</label><div class="col-sm-9">
       <input name="name" type="text" id="form-field-1" placeholder="" class="col-xs-10 col-sm-5" value="" >      </div></li>
       <li><label class="label_name">菜品分类</label>
       	<div class="col-sm-9">
       		<? sort_list1($fen_id);?>
       	</div></li>
      <li><label class="label_name">打印机说明</label><div class="col-sm-9"><textarea name="beizhu" class="form-control" id="form-field-8" placeholder="" onkeyup="checkLength(this);"></textarea><span class="wordage">剩余字数：<span id="sy" style="color:Red;">200</span>字</span></div></li>
...
function member_del(obj,id){
	layer.confirm('删除分类对应的该分类下的商品也会被删除,确认要删除吗？',{icon:0,},function(index){
